...
def test_GribFile(tmp_path):
    ...
    gfile = mrms.download.GribFile(tstamp)
    mrms.download.single_file(gfile)
    assert gfile.exists()
...
def test_mrms_datatypes(tmp_path):
    ...
    for dtype in get_args(mrms.typing_utils.MRMSDataType):
        gfile = mrms.download.GribFile(tstamp, data_type=dtype)
        mrms.download.single_file(gfile)
        assert gfile.exists()
...
def test_download_range(tmp_path):
    ...
    gfiles = mrms.download.timerange(init_tstamp, end_tstamp)
    for gf in gfiles:
        assert gf.exists()
